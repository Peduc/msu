...
for j=2:M
    for i=2:N
        a_newt=-6*tau*(t(j)+tau/2)/4;
        b_newt= 1+alpha(j);
        c_newt=(y(j,i-1)-y(j-1,i))*(1-alpha(j))-y(j-1,i-1)*(1+alpha(j))-...
                6*tau*(t(j)+tau/2)*(y(j,i-1)^2+y(j-1,i)^2+y(j-1,i-1)^2)/4;
      newt_old=y(j-1,i-1);
      for k=1:count         
          newt_new=newt_old-(a_newt*newt_old^2+b_newt*newt_old+c_newt)/...
                   (2*a_newt*newt_old+b_newt);
